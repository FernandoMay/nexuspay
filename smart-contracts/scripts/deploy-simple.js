const { AccountId, PrivateKey, ContractCreateFlow, Client, AccountBalanceQuery } = require("@hashgraph/sdk");
const fs = require("fs");
const path = require("path");

async function main() {
  console.log("🚀 Deploying SimpleToken using Hedera SDK...");
  ...
  const artifactsPath = path.join(__dirname, "../artifacts/contracts/SimpleToken.sol/SimpleToken.json");
  ...
  try {
    ...
    if (receipt.status.toString() === "SUCCESS") {
      ...
      fs.writeFileSync(
        "./deployments/simple-token-testnet.json",
        JSON.stringify(deploymentInfo, null, 2)
      );
      
      console.log("📁 Deployment info saved to: ./deployments/simple-token-testnet.json");
